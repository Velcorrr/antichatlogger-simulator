...
export function createOperator(root, ctx) {
  ...
  root.innerHTML = `<div class="o-viewport"></div><div class="o-grain"></div>
    ...
    <div class="o-chat-log"></div><form class="o-chat-form" hidden><label>TEAM</label><input maxlength="140" aria-label="Operator team chat" placeholder="Message your team…" autocomplete="off"><small>ENTER SEND · ESC CLOSE</small></form>
  ...
  function makeScene() {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.35));renderer.setClearColor(0x101816);renderer.outputColorSpace=THREE.SRGBColorSpace;
  ...
  function endRound(winner,reason){if(stage!=='live')return;score[winner]++;stage='roundend';stageTime=5;mouseDown=false;objective=0;ui.interact.textContent='';
  ...
  function hurt(victim,damage,killer,headshot=false){if(!victim.alive)return;if(victim===player){let absorb=Math.min(player.armor,damage*.40);player.armor-=absorb;damage-=absorb;damageTime=.5;audio('hit');}victim.hp-=damage;if(victim.hp>0)return;
    victim.hp=0;victim.alive=false;victim.deaths++;killer.kills++;feed(killer.name,victim.name,headshot);if(victim===player){matchDeaths++;stats.deaths=(Number(stats.deaths)||0)+1;mouseDown=false;spectate=0;ui.spectator.hidden=false;ctx.save?.();}else{victim.group.rotation.z=Math.PI/2;victim.group.position.y=-.08;victim.body.visible=false;victim.head.visible=false;}
  ...
  function fire(){if(stage!=='live'||!player.alive||reloading>0||shotTime>0||!visible)return;if(ammo<=0){reload();return;}ammo--;shotTime=weapon.interval;recoil=Math.min(.11,recoil+weapon.recoil);flash.visible=true;audio('shot');
    camera.getWorldDirection(direction);const spread=(aiming?.002:.010)+(keys.has('ShiftLeft')?.045:0)+recoil*.16;direction.x+=(Math.random()-.5)*spread;direction.y+=(Math.random()-.5)*spread;ray.set(camera.position,direction.normalize());ray.far=60;
  ...
  function updateCamera(dt){if(!camera)return;let cp=player.pos;
    if(!player.alive){const friends=bots.filter(b=>b.team===0&&b.alive);if(friends.length){const target=friends[spectate%friends.length];cp=target.pos;camera.position.set(cp.x,1.52,cp.z);const looking=team.filter(t=>t.team===1&&t.alive).sort((a,b)=>a.pos.distanceToSquared(cp)-b.pos.distanceToSquared(cp))[0];if(looking)camera.lookAt(looking.pos.x,1.25,looking.pos.z);ui.spectator.innerHTML=`<small>YOU ARE DEAD / SPECTATING</small><b>${esc(target.name)}</b><span>SPACE next teammate · F2 Discord</span>`;}else camera.position.set(player.pos.x,.7,player.pos.z);gun.visible=false;return;}
    const moving=visible&&(keys.has('KeyW')||keys.has('KeyA')||keys.has('KeyS')||keys.has('KeyD'));const crouch=keys.has('KeyC')||keys.has('ControlLeft');gunBob+=dt*(moving?keys.has('ShiftLeft')?13:9:2);camera.position.set(cp.x,crouch?1.05:1.62,cp.z);camera.position.y+=moving?Math.sin(gunBob)*.035:0;camera.rotation.set(pitch,yaw,0);camera.fov=THREE.MathUtils.lerp(camera.fov,aiming?58:keys.has('ShiftLeft')&&moving?84:78,.15);camera.updateProjectionMatrix();gun.visible=true;
    gun.position.set(aiming?.015:.27,-.27+(moving?Math.sin(gunBob)*.012:Math.sin(clock*1.7)*.004)-(reloading>0?.22:0),-.48+recoil*.65);gun.rotation.z=reloading>0?-.5:0;gun.rotation.x=reloading>0?-.28:recoil;}
  function update(dt){dt=Math.min(dt,.1);clock+=dt;if(!running){if(visible&&renderer){updateCamera(dt);renderer.render(scene,camera);}return;}
    ...
    if(stage==='queue'){stageTime-=dt;if(stageTime<=0)startRound();}
      ...
      if(player.alive&&visible&&document.pointerLockElement===renderer?.domElement&&!chatOpen()){
        const speed=(keys.has('KeyC')||keys.has('ControlLeft'))?1.8:keys.has('ShiftLeft')?5.8:3.4;let dx=(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),dz=(keys.has('KeyS')?1:0)-(keys.has('KeyW')?1:0);const len=Math.hypot(dx,dz)||1;dx/=len;dz/=len;moveBody(player.pos,(Math.cos(yaw)*dx+Math.sin(yaw)*dz)*speed*dt,(-Math.sin(yaw)*dx+Math.cos(yaw)*dz)*speed*dt);if(mouseDown)fire();
        if(player.pos.distanceTo(objectivePos)<2.6){ui.interact.innerHTML=`HOLD <b>E</b> TO EXTRACT DATA <div class="o-progress"><i style="width:${objective/4*100}%"></i></div>`;if(keys.has('KeyE')){objective+=dt;if(objective>=4)endRound(0,'Data extracted successfully');}else objective=Math.max(0,objective-dt*.4);}else{objective=0;if(!reloading)ui.interact.textContent='';}
      }
    ...
    if(visible&&renderer){updateCamera(dt);renderer.render(scene,camera);ui.hitmarker.style.opacity=hitTime>0?1:0;ui.damage.style.opacity=damageTime>0?damageTime*1.3:0;ui.crosshair.style.setProperty('--o-spread',`${6+recoil*150}px`);updateObjective();ui.resume.hidden=stage!=='live'||document.pointerLockElement===renderer.domElement||chatOpen()||!player.alive;}
  }
  ...
  function openChat(){ui['chat-form'].hidden=false;unlock();ui['chat-form'].querySelector('input').focus();keys.clear();}
  function closeChat(){ui['chat-form'].hidden=true;ui['chat-form'].querySelector('input').blur();}
  function lock(){if(!renderer||!visible||!running||stage==='finished'||stage==='queue')return;renderer.domElement.requestPointerLock?.()?.catch?.(()=>{});}
  function unlock(){keys.clear();mouseDown=false;aiming=false;if(renderer&&document.pointerLockElement===renderer.domElement)document.exitPointerLock?.();}
  function show(){visible=true;root.hidden=false;root.style.display='';resize();}
  function hide(){visible=false;root.hidden=true;root.style.display='none';unlock();closeChat();ui.scoreboard.hidden=true;}
  function resize(){if(!renderer)return;const w=root.clientWidth||window.innerWidth,h=root.clientHeight||window.innerHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();}
  ...
  on(window,'blur',()=>{keys.clear();mouseDown=false;});
  on(document,'pointerlockchange',()=>{if(document.pointerLockElement!==renderer?.domElement){keys.clear();mouseDown=false;aiming=false;}});
  on(window,'mousemove',e=>{if(!visible||document.pointerLockElement!==renderer?.domElement||!player.alive)return;const sensitivity=.0022*(Number(ctx.state.settings?.sensitivity)||1);yaw-=e.movementX*sensitivity;pitch=clamp(pitch-e.movementY*sensitivity,-1.25,1.25);});
  on(root,'mousedown',e=>{if(document.pointerLockElement!==renderer?.domElement)return;if(e.button===0)mouseDown=true;if(e.button===2)aiming=true;});
